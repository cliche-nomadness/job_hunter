...
- **对话式入口**：LLM function calling 做意图识别与参数抽取，自然语言直达各功能（现有命令保留为精确通道与降级兜底）
- **投递状态自动化**：IMAP 拉取招聘系统邮件（笔试/面试邀约），自动推进状态机并主动提醒——从"手动记录"演进为"邮件驱动"
- **消息卡片化**：简历意见 / 优化报告改交互卡片输出（摘要 + 确认按钮），完整报告落盘存档
- **LLM 精排推荐**：关键词粗筛之上叠加 DeepSeek 岗位匹配打分，只推送高相关岗位
- **数据层升级**：jobs.csv → SQLite，根治多进程并发写与数据量增长问题
